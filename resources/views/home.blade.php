        ...
        @foreach($products as $product)
            <div class="lg:w-1/4 md:w-1/2 p-4 w-full hover:bg-[#c8c8c8] transition-all duration-200 border rounded-tl-xl">
                <a class="block relative h-48 rounded overflow-hidden">
                    <img alt="ecommerce" class="object-cover object-center w-full h-full block" src="{{ $product['gambar_produk'] }}">
                </a>
                <div class="mt-4">
                    <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">{{ $product['merk_produk'] }}</h3>
                    <h2 class="text-gray-900 title-font text-lg font-medium">{{ $product['kategori_produk'] }}</h2>
